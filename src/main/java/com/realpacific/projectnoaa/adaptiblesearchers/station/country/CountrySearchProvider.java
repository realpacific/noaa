...
public class CountrySearchProvider implements SearchProvider<Station> {

    @Override
    public Searcher<String, Station> get(DatabaseService service) {
        if (!(service instanceof StationService)) throw new IllegalArgumentException("Invalid argument. Must be of type StationService.");
        return new DatabaseCountrySearcher((StationService) service);
    }

    @Override
    public Searcher get(List<Station> stations) {
        return new InMemoryCountrySearcher(stations);
    }
}
